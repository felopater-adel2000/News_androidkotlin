...
@HiltViewModel
class SearchViewModel @Inject constructor(val useCase: NewsUseCase) : ViewModel()
{
    ...
    fun searchAbout(name: CharSequence)
    {
        printMessage("Search for article")
        printMessage(name.toString())
        viewModelScope.launch {

            _visibilityState.value = State.Loading

            _articles.value = useCase.searchForArticles(name.toString())
